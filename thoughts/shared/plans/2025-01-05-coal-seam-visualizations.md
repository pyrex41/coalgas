# Coal Seam Visualization Implementation Plan

## Overview

Create static and interactive visualizations of coal seam thickness and depth data for all seams (Herrin, Springfield, Danville, Colchester, Seelyville) in the 6-county study area. Visualizations will help identify optimal CBM drilling locations and highlight data quality issues.

## Current State Analysis

### Available Data

**Drill Hole Data** (`data/csv/all-coals-study-area-combined.csv`):
- 2,854 records with lat/lon coordinates
- Columns per seam: `TOP_*` (elevation ft), `THICK_*` (thickness ft)
- `SURFELV` for calculating actual depth from surface

**County Boundaries** (`data/shapefiles/IL_BNDY_County_Py.shp`):
- Illinois county polygons for overlay

### Key Discoveries
- Thickness values range ~0.5-8 ft for most seams
- Depth calculated as: `SURFELV - TOP_*`
- Optimal CBM depth range: 500-1000 ft from surface
- Some records have missing values (empty strings)
...
├── visualize_coal_data.py            # Main visualization script
├── visualizations/
│   ├── static/
│   │   ├── herrin-thickness-map.png
│   │   ├── herrin-depth-map.png
│   │   ├── herrin-histogram.png
│   │   ├── springfield-thickness-map.png
│   │   ├── springfield-depth-map.png
│   │   ├── springfield-histogram.png
│   │   ├── danville-thickness-map.png
│   │   ├── danville-depth-map.png
│   │   ├── danville-histogram.png
│   │   ├── colchester-thickness-map.png
│   │   ├── colchester-depth-map.png
│   │   ├── colchester-histogram.png
│   │   ├── seelyville-thickness-map.png
│   │   ├── seelyville-depth-map.png
│   │   ├── seelyville-histogram.png
│   │   └── all-seams-summary.png
│   └── interactive/
│       ├── herrin-map.html
│       ├── springfield-map.html
│       ├── danville-map.html
│       ├── colchester-map.html
│       ├── seelyville-map.html
│       └── all-seams-map.html
...
### Verification
- [ ] All PNG files render correctly and show expected data
- [ ] All HTML files open in browser with working interactivity
- [ ] County boundaries visible on all maps
- [ ] Outliers flagged/handled appropriately
- [ ] Color scales are sequential and readable

## What We're NOT Doing

- 3D visualizations
- Animated visualizations
- Web server/dashboard deployment
- Contour interpolation (using point data only, not polygon contours)
- Gas content estimation (no data available)
...
### Overview
Set up uv virtual environment and install required dependencies.
...
name = "coalgas"
version = "0.1.0"
description = "Crawford County CBM data acquisition and visualization"
...
- [x] `uv run python -c "import matplotlib, folium, shapely, shapefile"` succeeds
...
### Overview
Create functions to load, validate, and filter coal data with outlier detection.

### Changes Required:

#### 1. Create visualization script with data loading
**File**: `visualize_coal_data.py`
...
#### Automated Verification:
- [x] Script parses without syntax errors: `uv run python -m py_compile visualize_coal_data.py`
- [x] Data loads successfully: `uv run python -c "from visualize_coal_data import load_drill_holes; h, s = load_drill_holes('data/csv/all-coals-study-area-combined.csv', 'herrin'); print(f'{len(h)} holes loaded')"`
...
## Phase 3: Static Visualizations (matplotlib)

### Overview
Add functions to generate static PNG maps and histograms using matplotlib.

### Changes Required:

#### 1. Add county boundary loading
**File**: `visualize_coal_data.py` (append)
...
#### 2. Add matplotlib plotting functions
**File**: `visualize_coal_data.py` (append)
...
#### Automated Verification:
- [x] Script parses without errors: `uv run python -m py_compile visualize_coal_data.py`
- [x] County boundaries load: `uv run python -c "from visualize_coal_data import load_county_boundaries; c = load_county_boundaries('data/shapefiles/IL_BNDY_County_Py.shp'); print(f'{len(c)} counties')"`
...
## Phase 4: Interactive Visualizations (folium)

### Overview
Add functions to generate interactive HTML maps using folium (Leaflet).

### Changes Required:

#### 1. Add folium map generation
**File**: `visualize_coal_data.py` (append)
...
- [x] Folium import works: `uv run python -c "import folium; print(folium.__version__)"`
...
## Phase 5: Main Execution and Output

### Overview
Add main execution block to generate all visualizations and data quality report.

### Changes Required:

#### 1. Add main execution
**File**: `visualize_coal_data.py` (append)
...
#### 2. Create output directories gitkeep
```bash
mkdir -p visualizations/static visualizations/interactive
touch visualizations/static/.gitkeep visualizations/interactive/.gitkeep
```

### Success Criteria:

#### Automated Verification:
- [x] Full script runs without errors: `uv run python visualize_coal_data.py`
- [x] Static PNG files created: `ls visualizations/static/*.png | wc -l` (should be 15)
- [x] Interactive HTML files created: `ls visualizations/interactive/*.html | wc -l` (should be 6)
- [x] Data quality report created: `cat data-quality-report.txt`

#### Manual Verification:
- [ ] Open PNG files - maps show data points with county boundaries
- [ ] Open PNG files - color scales are readable and sequential
- [ ] Open HTML files in browser - markers are clickable with popups
- [ ] HTML layer controls work - can toggle seams on/off
- [ ] Data quality report shows reasonable numbers (no excessive outliers)

**Implementation Note**: After all automated checks pass, pause here for manual verification before committing.
...
```bash
# Syntax check
uv run python -m py_compile visualize_coal_data.py

# Quick functional test
uv run python -c "
from visualize_coal_data import load_drill_holes, load_county_boundaries, SEAMS
holes, stats = load_drill_holes('data/csv/all-coals-study-area-combined.csv', 'herrin')
print(f'Herrin: {stats}')
counties = load_county_boundaries('data/shapefiles/IL_BNDY_County_Py.shp')
print(f'Counties: {len(counties)}')
"

# Full generation
uv run python visualize_coal_data.py
```

### Manual Testing Steps
1. Open `visualizations/static/herrin-thickness-map.png` - verify county boundaries visible
2. Open `visualizations/static/springfield-depth-map.png` - verify green points in optimal range
3. Open `visualizations/interactive/herrin-map.html` in browser - click markers, verify popups
4. Open `visualizations/interactive/all-seams-map.html` - toggle layers on/off
5. Review `data-quality-report.txt` - check outlier counts are reasonable

---

## Performance Considerations

- **MarkerCluster** used in folium to handle 2,800+ points efficiently
- Static PNG files saved at 150 DPI (good quality without huge file size)
- County boundaries loaded once and reused for all maps
...
- Source data: `data/csv/all-coals-study-area-combined.csv`
- County shapefile: `data/shapefiles/IL_BNDY_County_Py.shp`
